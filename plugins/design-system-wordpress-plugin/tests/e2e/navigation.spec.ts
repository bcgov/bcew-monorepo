...
const requestRestAsCurrentUser = async (
    ...
    return page.request.fetch( `/index.php?rest_route=${ path }`, {
        method,
        data,
 ...
 * @param {Object} nav      - The navigation locator.
 * @param {string} linkText - The submenu link text to find.
 * @return {Promise<Object>} Promise resolving to submenu and container locators.
 */
const findSubmenuContainer = async ( nav: any, linkText: string ) => {
    // Find the link first
    ...
    test.describe( 'Navigation Links', () => {
        ...
        test( 'Active/current page link is properly highlighted', async ( {
            ...
            await requestUtils.rest( {
                ...
                path: `/wp/v2/pages/${ testPage.id }`,
                params: {
                    content: `<!-- wp:design-system-wordpress-plugin/navigation {"ref":${ menuId },"menuId":${ menuId }} /-->`,
                },
            } );

            // Navigate to the test page
            const preview = await editor.page.context().newPage();

            // Navigate to the page
            await preview.goto( pageResponse.link, {
 ...
 *
 * @param {RequestUtils} requestUtils   - RequestUtils instance for API calls.
 * @param {Object}       menuData       - Menu configuration.
 * @param {string}       menuData.title - Menu title.
 * @param {Array}        menuData.items - Menu item definitions.
 * @return {Promise<number>} Promise resolving to the menu ID.
 */
const createNavigationMenu = async (
     ...
     * @param {string} title - Navigation link title.
     * @param {string} url   - Navigation link URL.
     * @return {string} Serialized navigation link block.
     */
    const serializeLinkBlock = ( title: string, url: string ): string => {
     ...
     * @param {string} title    - Navigation submenu title.
     * @param {string} url      - Navigation submenu URL.
     * @param {Array}  children - Child navigation item definitions.
     * @return {string} Serialized navigation submenu block.
     */
    const serializeSubmenuBlock = (
        title: string,
        url: string,
        children: Array< {
            title: string;
 ...
 * @param {number} menuId - Menu ID to select.
 * @return {Promise<void>} Promise that resolves after the menu is selected.
 */
const insertNavigationBlock = async (
    editor: Editor,
    menuId: number
): Promise< void > => {
 ...
 *
 * @param {Editor} editor - Editor instance.
 * @param {string} mode   - Overlay mode: 'always', 'mobile', or 'never'.
 * @return {Promise<void>} Promise that resolves after the mode is selected.
 */
const setOverlayMode = async (
    editor: Editor,
    ...
    await editor.page.evaluate(
        ( [ id, overlayMenu ] ) => {
            ( window as any ).wp.data
                ...
                .updateBlockAttributes( id, { overlayMenu } );
        },
        [ clientId, mode ]
    );

    await expect( navBlock ).toHaveClass(
        new RegExp( `dswp-block-navigation-is-${ mode }-overlay` )
    );
};
 ...
 *
 * @param {Editor}  editor      - Editor instance.
 * @param {string}  settingName - Name of the setting (e.g., 'Show in Desktop').
 * @param {boolean} value       - Boolean value to set.
 * @return {Promise<void>} Promise that resolves after the setting is updated.
 */
const setNavigationSetting = async (
    editor: Editor,
    settingName: string,
    value: boolean
): Promise< void > => {
 ...
 *
 * @param {Editor} editor - Editor instance.
 * @return {Promise<void>} Promise that resolves after the attributes are updated.
 */
const setNavigationVisibilityBothDesktopAndMobile = async (
    editor: Editor
): Promise< void > => {
    const navBlock = editor.canvas.locator(
        '[data-type="design-system-wordpress-plugin/navigation"]'
 ...
 *
 * @param {Editor} editor     - Editor instance.
 * @param {number} breakpoint - Breakpoint value in pixels.
 * @return {Promise<void>} Promise that resolves after the breakpoint is updated.
 */
const setMobileBreakpoint = async (
    editor: Editor,
    breakpoint: number
): Promise< void > => {
    await editor.openDocumentSettingsSidebar();
