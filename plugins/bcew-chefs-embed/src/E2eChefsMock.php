...
class E2eChefsMock {
    ...
    public static function pre_http_request( $pre, $args, $url ) {
        ...
        return $is_auth_request || $is_form_request ? self::mocked_response( $args, $is_form_request ) : $pre;
    }

    /**
     * Build a response for a supported CHEFS request.
     *
     * @param array $args            HTTP request arguments.
     * @param bool  $is_form_request Whether the request asks for form metadata.
     * @return array
     */
    private static function mocked_response( $args, $is_form_request ) {
        ...
        return $form_exists && $api_key_valid ? [
            'body'     => wp_json_encode(
                $is_form_request ? [
                    'title'    => 'E2E test form',
                    'versions' => [
