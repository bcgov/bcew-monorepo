...
This project uses Playwright with `@wordpress/e2e-test-utils-playwright` for end-to-end tests that run against a live WordPress environment. Tests are located in `themes/bcew-theme/tests/e2e/`.

```bash
npx nx run bcew-theme:wp-env-start
npx nx run bcew-theme:test-e2e
```
...
```bash
npx nx run bcew-theme:wp-env-start
(cd themes/bcew-theme && npx playwright test tests/e2e/copyright-shortcode.spec.ts)
```

To step through each test in headed mode for debugging:

```bash
npx playwright test themes/bcew-theme/tests/e2e/ --debug
```
...
```bash
npx nx run bcew-theme:wp-env-start
npx nx run bcew-theme:test-screenshot
```

**Note**: When creating a new pattern it must be added to `themes/bcew-theme/tests/screenshot/patterns.spec.js` in order to be included in regression tests.
...
```bash
npx nx run bcew-theme:wp-env-start
npx nx run bcew-theme:test-screenshot-generate
```
...
```bash
npx nx run bcew-theme:wp-env-start
npx playwright test themes/bcew-theme/tests/e2e/css-props.spec.ts
```
...
```bash
npx nx run bcew-theme:wp-env-start
npx playwright test themes/bcew-theme/tests/e2e/hero-variation.spec.ts
```
...
```html
<!-- wp:template-part {"slug":"breadcrumb","align":"full","theme":"bcew-theme"} /-->
```
...
Run `pnpm lint-template-parts` from the monorepo root (or `npx nx run bcew-theme:lint-template-parts` for this theme only) to validate locally. This check also runs automatically on pull requests as part of the `Lint monorepo` workflow job.
