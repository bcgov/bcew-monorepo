...
- Provides a BCEW theme implementation for TI Corp sites.
...
1. Start the theme test environment (optional - the nx target will run this automatically):
	...
	```bash
	npx nx run bcew-ticorp:wp-env-start
	```

1. Run all visual regression tests:

	```bash
	npx nx run bcew-ticorp:test-screenshot
	```

1. Run one visual regression spec file:

	```bash
	npx nx run bcew-ticorp:test-screenshot -- ./tests/screenshot/example.spec.js
	```

1. Run one visual regression spec file in Playwright debug mode:

	```bash
	npx nx run bcew-ticorp:test-screenshot -- --debug ./tests/screenshot/example.spec.js
	```
