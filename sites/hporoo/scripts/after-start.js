...
let HOME_PAGE_CONTENT = PAGE_CONTENT;
...
if (!postsPageId) {
  postsPageId = wp(`post create --post_type=page --post_title="${POSTS_PAGE_TITLE}" --post_name="${POSTS_PAGE_SLUG}" --post_status=publish --porcelain`).trim();
} else {
  wp(`post update ${postsPageId} --post_status=publish`);
}

// Keep a stable secondary page for local navigation and manual testing.
let samplePageId = wp(`post list --post_type=page --name=sample-page --post_status=publish,draft,pending,future,private --field=ID --format=ids`).trim();
if (!samplePageId) {
  samplePageId = wp(`post create --post_type=page --post_title="Sample Page" --post_name="sample-page" --post_status=publish --porcelain`).trim();
}
...
wp(`eval '$nav_id = ${PRIMARY_NAV_ID}; $sample_page_id = ${samplePageId};'`);
