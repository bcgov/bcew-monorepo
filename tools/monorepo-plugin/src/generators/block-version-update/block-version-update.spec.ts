...
describe( 'block-version-update generator', () => {
    ...
    beforeEach( () => {
        ...
        tree.write( 'plugins/test-plugin/src/sample-block/block.json', JSON.stringify( { version: '1.0.0' }, null, 2 ) );
        tree.write( 'plugins/another-plugin/src/another-block/block.json', JSON.stringify( { version: '1.0.0' }, null, 2 ) );
    } );

    it( 'should update all block.json version fields', async () => {
        await blockVersionUpdateGenerator( tree, options );

        const firstBlockJson = JSON.parse( tree.read( 'plugins/test-plugin/src/sample-block/block.json' )!.toString() );
        const secondBlockJson = JSON.parse( tree.read( 'plugins/another-plugin/src/another-block/block.json' )!.toString() );
    ...
    it( 'should handle missing plugins directory gracefully', async () => {
        ...
        await expect( blockVersionUpdateGenerator( emptyTree, options ) ).rejects.toThrow( 'No block.json files found to update.' );
    } );
} );
